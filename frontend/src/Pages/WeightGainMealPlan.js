  ...
  useEffect(() => {
    const fetchMealPlan = async () => {
      try {
        ...
        console.log('Fetching weight gain meal plan for user:', userId);
        ...
        console.error("Error fetching weight gain meal plan:", err.response ? err.response.data : err.message);
        setError("Failed to load weight gain meal plan. Please try again later.");
      }
    };
  ...
  return (
    <>
      <Container className="mt-4 meal-plan-container">
        <h1>Your Weight Gain Meal Plan</h1>
        ...
        {mealPlan ? (
          ...
          <p>Loading weight gain meal plan...</p>
        )}
      </Container>
    </>
  );
