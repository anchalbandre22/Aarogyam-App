...
const WeightLossMealPlanPage = () => {
    ...
    useEffect(() => {
        const fetchMealPlans = async () => {
            try {
                ...
                console.log('Fetching weight loss meal plans for user:', userId);
                ...
                console.error("Error fetching weight loss meal plans:", err.response ? err.response.data : err.message);
                setError("Failed to load weight loss meal plans. Please try again later.");
            }
        };
    ...
    return (
        <Container className="mt-4 meal-plan-container">
            <h1>Your Weight Loss Meal Plans</h1>
            ...
            {mealPlans.length > 0 ? (
                ...
                <p>No weight loss meal plans found.</p>
            )}
        </Container>
    );
};
